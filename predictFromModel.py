...
class prediction:
    ...
    def predictionFromModel(self):
        ...
        try:
            ...
            clusters=kmeans.predict(data.drop(['CustomerID'], axis=1))  # drops the first column for cluster prediction
            data['clusters'] = clusters
            clusters = data['clusters'].unique()
            for i in clusters:
                cluster_data = data[data['clusters'] == i]
                customer_ids = list(cluster_data['CustomerID'])
                cluster_data = data.drop(labels=['CustomerID'], axis=1)
                cluster_data = cluster_data.drop(['clusters'], axis=1)
                model_name = file_loader.find_correct_model_file(i)
                model = file_loader.load_model(model_name)
                result = list(model.predict(cluster_data))
                result = pandas.DataFrame(list(zip(customer_ids,result)), columns=['CustomerID', 'Prediction'])
